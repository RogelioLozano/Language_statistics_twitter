...
for country in countries:
    for admin_level in levels:
        for region in admins[country][admin_level]:
            ...
            if admin_level == -1:
                path = path = os.path.join(os.getenv("HOME"),'Adminlevel_filtered_data','normalized_case','Countries','iztapalapa_normalized',country,'')
                output_folder = os.path.join(os.getenv("HOME"),'Adminlevel_filtered_data','normalized_case','Countries','iztapalapa_normalized','Formatted_data',country,'')
            else:
                # path = os.path.join(os.getenv("HOME"),'Adminlevel_filtered_data','normalized_case','Countries',country,'Level_{}'.format(admin_level),'')
                path = os.path.join(os.getenv("HOME"),'Adminlevel_filtered_data','normalized_case','iztapalapa_normalized',country,'Level_{}_{}'.format(admin_level,region),'')
                
                # output_folder = os.path.join(os.getenv("HOME"),'Adminlevel_filtered_data','normalized_case','Countries','Formatted_data',country,'Level_{}'.format(admin_level))
                output_folder = os.path.join(os.getenv("HOME"),'Adminlevel_filtered_data','normalized_case','iztapalapa_normalized','Formatted_data',country,'Level_{}_{}'.format(admin_level,region),'')

            # files=sorted(os.listdir('../../../storage/gershenson_g/gershenson/Ranks_15-10-18/Filtrados/'+country+'/'))
            ...
            for interval in interval_tweets:   
                # path='../../../storage/gershenson_g/gershenson/Ranks_15-10-18/Filtrados/Formatted_data/'+country+'/3hourly_csv_files/'
