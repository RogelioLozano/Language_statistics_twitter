...
data_path = '../../../storage/gershenson_g/gershenson/Ranks_15-10-18/Filtrados'
...
admins = {"Mexico":["iztapalapa_poly.geojson","miguel_hidalgo_poly.geojson"], "United_Kingdom":["london.geojson","borough_of_camden.geojson"]}
levels = [0,1]
...
for country in countries:
    for admin_level in levels:
        ...
        poly_path = os.path.join(os.getenv("HOME"),'polygons',country+'_polygons')
        ...
        for file in files: 
            with gzip.open(os.path.join(data_path,country,file), 'r') as f:
                ...
                # out_path = os.path.join(os.getenv("HOME"),'Adminlevel_filtered_data',country,'Level_{}'.format(admin_level),'')
                out_path  = os.path.join(os.getenv("HOME"),'Adminlevel_filtered_data',country,geopoly,'')
